...
func writeHostLine(b *strings.Builder, h app.HostCfg, snap *app.Snapshot, tasks map[string]projectTaskStat) {
	...
	fmt.Fprintf(b, "Host %q (%s): %d running, %d paused, %d queued, %d errors, run=%s net=%s, RAC %.0f, credit %.0f\n",
		h.Name, kind, snap.Totals.Running, snap.Totals.Paused, snap.Totals.Queued, snap.Totals.Errors,
		orDash(snap.TaskMode), orDash(snap.NetMode), snap.Totals.RAC, snap.Totals.Credit)
